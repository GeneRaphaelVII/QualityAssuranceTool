...
def withdraw(mode, list):
    ...
    if fromAccount not in list:
            print "Error: account does not exist"
    elif fromAccount != 0:
        amount = getAmount(mode)
        if amount != (-1):
                return writeTrans("02", "000000", fromAccount, amount, "               ")
        else:
                return 0
    else:
            return 0
                    ...
                    if mode == "agent" or mode == "retail":
                        ...
                        while loginState == 1:
                            ...
                            if mode == "agent" and command == "create":
                                ...
                                trans = withdraw(mode, accountsList)
                                if trans != 0:
                                        q.put(trans)
